...
class MyApp(ShowBase):
    ...
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture("record.mp4")
        ...
        # Set value for count frame.
        # I will get time using this value.
        self.frame_count = 0
        ...
        aspect_ratio = self.myMovieTexture.getVideoWidth() / self.myMovieTexture.getVideoHeight()  # 동영상 가로 세로 비율
...
def updateBg(task):
    ...
    if app.frame_count <= app.fps * TIME_INTERVAL and app.frame_count >= app.fps * TIME_DEALY:
        app.calibrateOn = True
        ...
        if found:
            ...
            corners2 = cv2.cornerSubPix(img_gray, corners, (30, 30), (-1, -1), app.criteria)
            ...
            # print(corners2[5][0][1],corners2[0][0][1])
            if distance > MIN_DISTANCE and corners2[5][0][1] > corners2[0][0][1]:
                # Update progress bar.
                app.textObject.text = f"calibrating count: {len(app.points2Ds):3d}"
    ...
    # If time over,
    # Set calibrateOn True.
    elif app.calibrateOn and app.frame_count > app.fps * TIME_DEALY:
        ...
        print("\nRMS:", rms_err)
        app.calibrateEnd(f"Calibration is done! RMS: {rms_err}")
        app.intrisic_mtx = intrisic_mtx
        app.dist_coefs = dist_coefs
        app.flag = False

        # Calculate aruco rms errors.
        mean_error = 0
        for i in range(len(app.points2Ds)):
            imgpoints2, _ = cv2.projectPoints(app.points3Ds[i], rvecs[i], tvecs[i], app.intrisic_mtx, app.dist_coefs)
            error = cv2.norm(app.points2Ds[i], imgpoints2, cv2.NORM_L2)/len(imgpoints2)
            mean_error += error
        print( "aruco error: {}".format(mean_error/len(app.points3Ds)) )

    # Undistort the frame if calibration is done.
    if app.calibrateOn and not app.flag:
