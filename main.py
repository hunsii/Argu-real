...
class MyApp(ShowBase):
    ...
    def init_webcam(self):
        ...
        self.imgW = int(self.video_capture.get(cv.CAP_PROP_FRAME_WIDTH))
        self.imgH = int(self.video_capture.get(cv.CAP_PROP_FRAME_HEIGHT))

    def init_background(self):
        winprops = p3c.WindowProperties()
        winprops.setSize(self.imgW, self.imgH)
        winprops.setOrigin(-1500, 200)
    ...
    def init_matarial(self):
        # 재질 생성 및 설정
        material = p3c.Material()
        material.diffuse = (1, 1, 1, 1)  # 오브젝트의 색상 설정
        material.ambient = (0.2, 0.2, 0.2, 1)  # 오브젝트의 주변광 설정
    ...
    def resetSBaseMouseCam(self):
        mat = p3c.Mat4(self.cam.getMat())
        mat.invertInPlace()
        self.mouseInterfaceNode.setMat(mat)
        self.mouseInterfaceNode.reset()

    def loadParameterFromFile(self):
        fs = cv.FileStorage("./labtopCameraParameters.txt", cv.FileStorage_READ)
        if not fs.isOpened(): # 파일 읽기 실패하면?
            raise IOError("Cannot open cam parameters")
        else:    
            # 읽기 성공 -> 칼리브레이션 했으니 내부 파라미터 읽으면 됨.
            self.intrisic_mtx = fs.getNode('camera intrinsic matrix').mat()
            self.dist_coefs = fs.getNode('distortion coefficients').mat()
        # retval, rvec, tvec=cv.solvePnP(figure_points_3D,imgPoints,self.cam_matrix,None,0)

    def GenerateFaceMesh(self):
        # note: use Geom.UHDynamic instead of Geom.UHStatic (resource setting for immutable or dynamic)
        vdata = p3c.GeomVertexData(
            'point_data', p3c.GeomVertexFormat.getV3t2(), p3c.Geom.UHDynamic)
        numVertices = 478
        # 4 refers to the number of cameras
        vdata.setNumRows(numVertices)
        vertex = p3c.GeomVertexWriter(vdata, 'vertex')
        texCoord = p3c.GeomVertexWriter(vdata, 'texcoord')
        #color = p3c.GeomVertexWriter(vdata, 'color')
        #normal = p3c.GeomVertexWriter(vdata, 'normal')
        ...
        for i in range(int(len(FACEMESH_TESSELATION) / 3)):
            primTris.addVertices(FACEMESH_TESSELATION[3 * i + 0][0], 
                                FACEMESH_TESSELATION[3 * i + 1][0], 
                                FACEMESH_TESSELATION[3 * i + 2][0])
        ...
        primPoints = p3c.GeomPoints(p3c.Geom.UHDynamic)
        primPoints.addConsecutiveVertices(0, numVertices)

        geom = p3c.Geom(vdata)
        #geom.addPrimitive(primPoints)
        geom.addPrimitive(primTris)
        geomNode = p3c.GeomNode("FaceMesh Landmark Points")
        geomNode.addGeom(geom)

        self.pointsVertex = vertex
        self.pointsTexCoord = texCoord
        #pbase.pointsColor = color
        #pbase.pointsNormal = normal
        self.points = p3c.NodePath(geomNode)
        self.points.setTwoSided(True)
        self.points.reparentTo(self.render)
        self.points.set_render_mode_thickness(5)
    ...
    def GenerateHandMesh(self):
        ...
        vertex.addData3f(-10002.0, -10002.0, -10002.0)
        vertex.addData3f(10000.0, 10000.0, 10000.0)
        texCoord.addData2f(0, 0)
        texCoord.addData2f(0, 0)
        ...
        primTris = p3c.GeomTriangles(p3c.Geom.UHStatic)
        for i in range(int(len(HANDMESH_TESSELATION) / 3)):
            primTris.addVertices(HANDMESH_TESSELATION[3 * i + 0][0], 
                                HANDMESH_TESSELATION[3 * i + 1][0], 
                                HANDMESH_TESSELATION[3 * i + 2][0])
        ...
        geom = p3c.Geom(vdata)
        geom.addPrimitive(primTris)
        geomNode = p3c.GeomNode("HandMesh Landmark Points")
        geomNode.addGeom(geom)

        self.pointsVertex2 = vertex
        self.pointsTexCoord2 = texCoord
        self.points2 = p3c.NodePath(geomNode)
        self.points2.setTwoSided(True)
        self.points2.reparentTo(self.render)
        self.points2.set_render_mode_thickness(5)
        
        myTextureStage = p3c.TextureStage("myTextureStage2")
        myTextureStage.setMode(p3c.TextureStage.MReplace)
        self.points2.setTexture(myTextureStage, self.tex)
...
def updateBg(task):
    success, frame = app.video_capture.read()
    if success == False:
        return task.cont
    ...
    #################
    if app.intrisic_mtx is not None:
        frame = cv.undistort(frame, app.intrisic_mtx, app.dist_coefs, None, app.intrisic_mtx)
    ...
    for face_landmarks in face_landmarks_list:
        # print(type(face_landmarks)) # List
        # print(len(face_landmarks)) # 478
        # print(face_landmarks[0]) 
        """
        NormalizedLandmark(x=0.4778910279273987, y=0.7691023349761963, z=-0.02604163996875286, visibility=0.0, presence=0.0)
        .x 이런식으로 접근한다.
        """
        app.pointsVertex.setRow(0)
        app.pointsTexCoord.setRow(0)
        point3d = p3c.LPoint3f()

        for landmark in face_landmarks:
            p = p3c.LPoint3f(landmark.x, landmark.y, landmark.z)
            # app.camLens.extrude(p3c.LPoint2f((p.x - 0.5) * 2.0, -(p.y - 0.5) * 2.0), nearPoint, farPoint)
            app.camLens.extrudeDepth(p3c.LPoint3f((p.x - 0.5) * 2.0, -(p.y - 0.5) * 2.0, p.z), point3d)
            matW = app.cam.getMat()
            # nWS = matW.xformPoint(nearPoint)
            nWS = matW.xformPoint(point3d)
            vec_cam2p = nWS - app.cam.getPos()
            pWS = app.cam.getPos() + vec_cam2p * 30
            app.pointsVertex.setData3f(pWS)
            # print(pWS)
            #app.pointsColor.setData4f(0, 0, 1, 1)
            app.pointsTexCoord.setData2f(p.x, 1 - p.y)
    ...
    if len(detection_result.hand_landmarks) > 0:
        ...
        # print(detection_result.hand_landmarks[0][0]) # NormalizedLandmark(x=0.7655252814292908, y=0.8937265276908875, z=-7.190344764040901e-09, visibility=0.0, presence=0.0)
        # print(detection_result.hand_world_landmarks[0][0]) # Landmark(x=0.0078347809612751, y=0.07009051740169525, z=0.027184970676898956, visibility=0.0, presence=0.0)
        # print('-'*80)
        app.pointsVertex2.setRow(0)
        ...
        for idx, landmark in enumerate(detection_result.hand_landmarks[0]):
            ...
            vec_cam2p = nWS - app.cam.getPos()
            pWS = app.cam.getPos() + vec_cam2p * 30
            app.pointsVertex2.setData3f(pWS)
            app.pointsTexCoord2.setData2f(p.x, 1 - p.y)

            if idx in [0, 5, 17]:
                point_list.append(pWS)
